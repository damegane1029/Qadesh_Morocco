...
//2020/6/12 Version 006 : Add no directional light mode.
//2020/6/12 Version 007 : Add ZWrite parameter.
...
Shader "Noriben/noribenQuestWaterCubemap"
{
	...
	{	
		...
		[PowerSlider(2)]_NormalPower ("WaveNormalPower", Range(0, 1)) = 0.01
		[PowerSlider(2)]_WaveHeight ("WaveHeight", Range(0, 5)) = 0.1
		...
		[Space]
		[Header(No Directional light mode)]
		[Toggle] _NoLight ("Custom light mode ON", int) = 0
		_LightCol ("Light Color" , Color) = (.5, 1.0, 1.0, 1.0)
		_LightDir ("Light Direction" , Vector) = (0, 1.0, 0, 1.0)
	...
	SubShader
	{
		...
		Pass
		{
			...
			{	
				...
				//ライティング
				//no light mode
				_LightColor0.rgb = lerp(_LightColor0.rgb, _LightCol.rgb, _NoLight);
				...
				//mix
				fixed4 col = _Color;
				col.rgb = _Color * (diffuse * _Diffuse) + (refColor.rgb * _Reflection) + (specular * _Specular);
				//no light mode
				fixed3 noLightCol = _Color.rgb * (diffuse * _Diffuse) + (refColor.rgb * _Reflection) + (specular * _Specular);
